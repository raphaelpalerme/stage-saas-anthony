...
- **🚨 STOP — ton dernier jour n'est PAS fini : il manque LE test utilisateur.** Tu as construit un réseau social complet en un jour — chapeau, mais **construire, tu sais déjà faire**. Ce qui te reste, c'est la compétence rare : **montrer Pickify à de VRAIS ados basketteurs** (pas le binôme, pas le tuteur), les laisser l'utiliser **sans les aider**, et écouter — surtout : **l'utiliseraient-ils vraiment pour trouver une partie ?** 30 min suffisent. Note 5 lignes **brutes** dans `livrables/retours-users.md`. **Tant que ce fichier est vide, le stage n'est pas bouclé.**
- *Avant de montrer* : vérifie que tout ce que tu as ajouté (chat, DM, feed) **marche vraiment bout en bout** — tu en as mis beaucoup en peu de temps, ne montre pas un truc cassé.
...
- **💬 Piste « rétention » — un chat une fois qu'on a rejoint une partie** : pour coordonner (« on se retrouve où ? à quelle heure ? ») et donner envie de revenir. ⚠️ Ça ne crée **PAS** d'effet réseau (ça n'amène aucun nouveau joueur) — c'est de la coordination/rétention, et un bel exercice technique (**Supabase Realtime** + table `messages` liée à la dispo, RLS « seuls les participants lisent/écrivent »).
- **📣 Piste « croissance / effet réseau » — partager sa dispo** : quand tu postes une dispo, un bouton **« Partager sur WhatsApp / Insta »** (lien + message pré-rempli) → tes potes cliquent, créent un compte, rejoignent, et ramènent les leurs. C'est **LE** vrai levier réseau (chaque user en amène d'autres), et ça part de là où sont déjà tes utilisateurs (les groupes WhatsApp = ton concurrent). En plus c'est **plus simple** à coder qu'un chat (un deep-link + un partage).
- **À toi de choisir UNE des deux** : si tu veux penser **croissance/produit** → le partage ; si tu veux un **bel artefact technique** pour le portfolio → le chat. Une seule, finie proprement.
